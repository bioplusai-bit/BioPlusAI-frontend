...
const STEPS: Step[] = [
  ...
  { key: 'SavingResults', label: 'Saqlash',        sub: 'VCF + DB yozilmoqda' },
  { key: 'Completed',     label: 'Tayyor',         sub: 'Natija ko\'rish mumkin' },
];

const ORDER: AnalysisStep[] = ['None','Uploading','DeepVariant','SavingResults','Completed'];
...
const ActiveDot = () => (
  <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#0f172a', animation: 'pulse 1.5s ease-in-out infinite' }} />
);
const Pulse = () => (
  <span style={{ display: 'inline-block', width: 7, height: 7, borderRadius: '50%', background: '#38bdf8', marginRight: 8, flexShrink: 0, animation: 'pulse 1.2s ease-in-out infinite' }} />
);

function circleStyle(s: Status): React.CSSProperties {
  if (s === 'done')    return { background: '#0ea5e9', color: '#fff', boxShadow: 'none' };
  if (s === 'active')  return { background: '#0f172a', border: '2px solid #0ea5e9', color: '#0ea5e9', boxShadow: '0 0 12px rgba(14,165,233,.4)' };
  if (s === 'failed')  return { background: '#ef4444', color: '#fff', boxShadow: 'none' };
  return { background: '#1e293b', border: '1px solid #334155', color: '#475569', boxShadow: 'none' };
}
