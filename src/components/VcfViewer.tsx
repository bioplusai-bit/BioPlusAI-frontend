...
export const VcfViewer: React.FC<Props> = ({ jobId, fileName, hasAmData }) => {
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={S.head}>
        <div style={S.headLeft}>
          ...
          <div>
            ...
            <div style={S.vcfMeta}>{loading ? 'Yuklanmoqda...' : `${variants?.length ?? 0} variant`}</div>
          </div>
        </div>
      ...
      {!loading && (
        <div style={S.tabs}>
          <button style={{ ...S.tab, ...(activeTab === 'table' ? S.tabActive : {}) }} onClick={() => setActiveTab('table')}>
            📊 Jadval
          </button>
          <button style={{ ...S.tab, ...(activeTab === 'vcf' ? S.tabActive : {}) }} onClick={() => setActiveTab('vcf')}>
            📄 VCF fayl
          </button>
        </div>
      )}
      ...
      {!loading && activeTab === 'table' && variants && (
        <div style={{ ...S.viewer, height: expanded ? 600 : 320 }}>
          <table style={S.table}>
            <thead>
              <tr>
                ...
                {hasHyena  && <th style={S.th}>Ishonch</th>}
                {hasHyena  && <th style={{ ...S.th, minWidth: 220 }}>Annotatsiya</th>}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={v.id} style={i % 2 === 0 ? {} : S.trAlt}>
                  ...
                  {hasHyena && (
                    <td style={{ ...S.td, textAlign: 'center' }}>
                      {v.hyenaConfidence != null && v.hyenaConfidence > 0 ? (
                        <div style={S.confWrap}>
                          <div style={{ ...S.confBar, width: `${v.hyenaConfidence * 100}%`, background: confColor(v.hyenaConfidence) }} />
                          <span style={S.confText}>{(v.hyenaConfidence * 100).toFixed(0)}%</span>
                        </div>
                      ) : <span style={S.na}>—</span>}
                    </td>
                  )}
                  {hasHyena && (
                    <td style={{ ...S.td, fontSize: 10, color: '#64748b', maxWidth: 260, whiteSpace: 'normal', lineHeight: 1.4 }}>
                      {v.hyenaAnnotation ?? <span style={S.na}>—</span>}
                    </td>
                  )}
                </tr>
              ))}
...
const S: Record<string, React.CSSProperties> = {
  ...
  confWrap:    { display: 'flex', alignItems: 'center', gap: 6, minWidth: 70 },
  confBar:     { height: 4, borderRadius: 2, flexShrink: 0, transition: 'width .3s' },
  confText:    { fontSize: 10, color: '#64748b', fontVariantNumeric: 'tabular-nums' },
};
