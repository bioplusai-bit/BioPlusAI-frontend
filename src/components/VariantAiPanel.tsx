...
export const VariantAiPanel: React.FC<Props> = ({ variant, onClose }) => {
  ...
  useEffect(() => {
    if (variant) {
      ...
      setTimeout(() => sendMessage(buildInitialMessage(variant), [], variant), 300);
    }
  ...
  const sendMessage = async (text: string, history: Message[], v: VariantRow) => {
    setLoading(true);
  ...
  const handleSend = () => {
    ...
    setInput('');
    sendMessage(text, messages, variant);
  };
  ...
  return (
    <>
      ...
      <div style={{ ...S.panel, transform: isVisible ? 'translateX(0)' : 'translateX(100%)', opacity: isVisible ? 1 : 0 }}>
        ...
        {messages.length > 1 && !loading && (
          <div style={S.suggestions}>
            {SUGGESTIONS.map(s => (
              <button key={s} style={S.suggBtn} onClick={() => { setInput(s); setTimeout(handleSend, 0); }}>{s}</button>
            ))}
          </div>
        )}
        ...
        <div style={S.inputWrap}>
          <input
            ...
            disabled={loading}
          />
          <button style={{ ...S.sendBtn, opacity: input.trim() && !loading ? 1 : 0.4 }} onClick={handleSend} disabled={!input.trim() || loading}>
      ...
      <style>{`
        @keyframes typingBlink { 0%,80%,100%{opacity:0;transform:scale(.8)} 40%{opacity:1;transform:scale(1)} }
      `}</style>
    </>
  );
};
